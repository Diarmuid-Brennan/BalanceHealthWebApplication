...
def create_patient():
		...
		if request.method == "POST":
			...
			#add_activities(userDetails["email"])
...
def edit_patient():
	if user["is_logged_in"] == True:
		...
		if request.method == "POST":
			...
			return render_template("edit_patient.html", data=data, patient_details=patient_details)
		return render_template("edit_patient.html", data=data, patient_details=patient_details)
	else:
		flash("You must be logged in to access webpage.", "error")
		return redirect(url_for('login'))
...
def patient_details():
	if user["is_logged_in"] == True:
		...
		#	activity_details = activity_details.replace("'", '"')
		#	activity = json.loads(activity_details)

		#	if activity['name'] in (x['name'] for x in (session["patient_activities"])):
		#		flash("User already has activity set", "error")
		#		return render_template(
		#			"patient_details.html",
		#			data=(session["patient_detail"]),
		#			activity_list=(session["activities"]),
		#			patient_activities=(session["patient_activities"]),
		#			)				
		#	else:
		#		add_patient_activity(activity, (session["user_email"]))
		#		patient_activities = get_patient_activities((session["user_email"]))
		...
		if request.method == "POST":
			...
			if details["activity"] == 'feetTogether':
				patient_comments = retrieve_comments("Stand with your feet side-by-side", user_email)
...
def view_activity_progress():
		...
		fig = px.bar(df, x="activityName", color="completed", barmode="group", text="date_set", title="Activities completed", labels=dict(count="Activities carried out"))
		...
		df = df[['activityName', 'date_set', 'max_value','min_value', 'avg_value', 'completed']]
...
def calculate_percentages(df):
	numInstepSuccess = df[(df['completed'] == True) & (df['activityName'] == 'Instep Stance')].shape[0]
	numInstep = df[(df['activityName'] == 'Instep Stance')].shape[0]
	
	numTademSuccess = df[(df['completed'] == True) & (df['activityName'] == 'Tandem Stance')].shape[0]
	numTandem = df[(df['activityName'] == 'Tandem Stance')].shape[0]
	
	numFeetTogetherSuccess = df[(df['completed'] == True) & (df['activityName'] == 'Stand with your feet side-by-side')].shape[0]
	numFeetTogether = df[(df['activityName'] == 'Stand with your feet side-by-side')].shape[0]
	
	numOneFootSuccess = df[(df['completed'] == True) & (df['activityName'] == 'Stand on one foot')].shape[0]
	numOneFoot = df[(df['activityName'] == 'Stand on one foot')].shape[0]
	
	percentages = [(round(((numFeetTogetherSuccess/numFeetTogether)*100),2)), (round(((numInstepSuccess/numInstep)*100),2)), (round(((numTademSuccess/numTandem)*100),2)), (round(((numOneFootSuccess/numOneFoot)*100),2))]
	return percentages
	

@app.route("/view_selected_activity/<activity>", methods=["GET", "POST"])
def view_selected_activity(activity):
#@app.route("/view_selected_activity", methods=["GET", "POST"])
#def view_selected_activity():
	#if user["is_logged_in"] == True:
		if request.method == "POST":
			if "user_email" in session:
				user_email = session["user_email"]
		...
		fig2 = px.sunburst(df.head(7), path=["date_set","completed"], hover_name = "activityName", color='completed')
		fig2.write_image("static/images/fig3.png")
		
		fig = px.line(df, x="date_set", y=["avg_value", "max_value"], title='Overall Average Score')
		fig.write_image("static/images/fig2.png")
		
		#fig1=Figure()
		#ax = fig1.add_subplot(1,1,1) 
		#ax = df.plot(x="date_set", y=["avg_value","max_value" ], kind="line")
		#image = io.BytesIO()
		#FigureCanvas(fig1).print_png(image)
		#chart = "data:image/png;base64,"
		#chart += base64.b64encode(image.getvalue()).decode('utf8')
		...
		return render_template("view_selected_activity.html",activities=activities, image=pngImageB64String)
